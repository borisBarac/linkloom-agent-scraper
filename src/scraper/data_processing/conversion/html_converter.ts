...
export const cleanHtmlToMarkdown = (html: string, options: CleanHtmlToMarkdownOptions = {}): string => {
  ...
  if (!article || !article.content) {
    ...
    throw new ScrapperError("ERR_MARKDOWN_CONTENT_EXTRACTION_FAILED", "Failed to extract readable content from HTML.");
  }
  ...
  let markdown: string;
  try {
    markdown = turndownService.turndown((article.title ?? "") + article.content);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error(`Turndown conversion failed: ${errorMessage}`);
    throw new ScrapperError("ERR_MARKDOWN_CONVERSION_FAILED", `Failed to convert HTML to Markdown: ${errorMessage}`);
  }

  if (!markdown || (typeof markdown === "string" && markdown.trim().length === 0)) {
    console.warn("Turndown conversion resulted in empty or non-string markdown content.");
  }
